...
![DeepTutor runtime](https://img.shields.io/badge/DeepTutor_runtime-not_required-2ea44f)
...
flowchart LR
    ...
    E --> H["Optional reviewed exact adapter"]
...
This repository is the **v0.6.0 RC2 Native Execution Core**. It restores the
reviewed v0.3 runtime invariants and is ready for physical integration into the
frozen v0.5 Host source. The RC2 baseline has **61/61 tests passing**; public
repository gates add three further tests.

The full merged v0.6 Host release is **not claimed**. The frozen v0.5 Host source
must be physically merged and the combined Host, browser, and production build
gates must pass before making that claim.
