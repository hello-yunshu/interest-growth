...
PY
  ...
  code="$(http POST /auth/owner/login -H "Content-Type: application/json" -d "{\"owner_password\":\"${OWNER_PASSWORD}\",\"device_name\":\"ci-recover\",\"platform\":\"android\",\"app_version\":\"1.0.5\"}")"
  if [ "${code}" = "201" ]; then
    PASS=$((PASS + 1))
    ACCESS_TOKEN="$(json_get "['tokens']['access_token']")"
