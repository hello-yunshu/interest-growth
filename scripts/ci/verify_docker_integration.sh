...
# 6. login
step "owner login"
code="$(http POST /auth/owner/login -H "Content-Type: application/json" -d "{\"owner_password\":\"${OWNER_PASSWORD}\",\"device_name\":\"ci-b\",\"platform\":\"android\",\"app_version\":\"1.0.6\"}")"
if [ "${code}" = "201" ]; then
  PASS=$((PASS + 1))
...
PY
  ...
  code="$(http POST /auth/owner/login -H "Content-Type: application/json" -d "{\"owner_password\":\"${OWNER_PASSWORD}\",\"device_name\":\"ci-recover\",\"platform\":\"android\",\"app_version\":\"1.0.6\"}")"
  if [ "${code}" = "201" ]; then
    PASS=$((PASS + 1))
    ACCESS_TOKEN="$(json_get "['tokens']['access_token']")"
