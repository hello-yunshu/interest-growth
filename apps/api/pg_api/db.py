...
class ServerMetadataModel(Base):
    ...
    backup/restore, and is never regenerated by migrations. The unique
...
def _record_migration(db, version: int) -> None:
    if db.get(SchemaMigration, version) is None:
        db.add(SchemaMigration(version=version))


def _migration_8_create_general_interest_schema(engine) -> None:
    # Explicit additive schema migration for v0.5. Existing v0.4.1 tables are untouched.
    for table in (
        DomainPackModel.__table__,
        MasteryProfileModel.__table__,
        InterestAreaModel.__table__,
        AreaCapabilitySettingModel.__table__,
        EntityAreaBindingModel.__table__,
        LearningActivityModel.__table__,
        GroundingRefModel.__table__,
    ):
        table.create(engine, checkfirst=True)
    # PersonaScope references tutor_personas, which exists in the legacy baseline.
    PersonaScopeModel.__table__.create(engine, checkfirst=True)


def _migration_9_seed_and_backfill(engine) -> None:
    # Domain packs and the default Psychology area are seeded by domain service after
    # migrations. This migration backfills legacy rows once the default area exists.
    from .domains import seed_domain_packs_and_default_area, backfill_legacy_area_bindings
    seed_domain_packs_and_default_area()
    backfill_legacy_area_bindings()


def _migration_10_plugin_aliases(engine) -> None:
    from .plugins import migrate_legacy_plugin_states
    migrate_legacy_plugin_states()


def _migration_11_native_execution_state(engine) -> None:
    """Install execution-only state required by the native v0.6 provider.

    Keep the statements here explicit so the host migration runner remains the
    only owner of persistent schema changes.  The native execution package
    deliberately refuses to create these tables for a persistent database.
    """
    statements = (
        """
...
def _migration_12_native_only_product(engine) -> None:
    """Retire the former compatibility provider and move legacy KBs to rebuildable native state."""
    with engine.begin() as connection:
        connection.execute(text("DELETE FROM plugin_states WHERE plugin_id = 'integration.deeptutor'"))
        connection.execute(text("DELETE FROM feature_flags WHERE name LIKE 'FEATURE_DEEPTUTOR_%'"))
...
def _migration_13_remote_auth(engine) -> None:
    """Add single-owner / device-session authentication tables (v0.7 remote mode).

    Strictly additive so existing v0.6 local databases upgrade in place. The
    tables are inert while PG_REMOTE_AUTH_ENABLED is unset, which keeps the
    existing desktop-local and loopback development contracts unchanged.
    """
    for table in (
        OwnerModel.__table__,
        DeviceModel.__table__,
        DeviceAccessTokenModel.__table__,
        DeviceRefreshTokenModel.__table__,
...
def _migration_15_server_identity(engine) -> None:
    """Add the single-row server identity (Gate C).

    Strictly additive so existing v0.7 databases upgrade in place: the table
    is created and the identity row backfilled exactly once. The identity is
    never regenerated by this or later migrations, is not derived from
    hostname/password/tokens, and is preserved across restarts and
    backup/restore because it lives in the database itself.
    """
    ServerMetadataModel.__table__.create(engine, checkfirst=True)
...
def get_server_identity() -> dict[str, str]:
    ...
    migration 15), never changes across restarts or backup/restore, and is
...
MIGRATIONS: dict[int, Any] = {
    ...
    10: _migration_10_plugin_aliases,
    ...
    15: _migration_15_server_identity,
}
...
def init_db(database_url: str | None = None) -> None:
    ...
    if fresh:
        ...
        # bootstrap. Historical 1-7 versions are recorded as the legacy baseline.
