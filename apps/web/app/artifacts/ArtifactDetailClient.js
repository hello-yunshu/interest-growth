...
import { CodeBlock, SafeSvgPreview, StatusChip, VisualExplanation } from '../../components/BeautifulUI';
...
export default function ArtifactDetailClient({ usePathParams = false, artifactId = '' }) {
  ...
  const [bundle, setBundle] = useState(null); const [visual, setVisual] = useState(null); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false);
  async function load() { if (!id) return; const next = await api(`/artifacts/${id}`); setBundle(next); if (next.artifact?.kind === 'visual_explanation') setVisual((await api(`/visual-artifacts/${id}/preview`)).manifest); else setVisual(null); }
