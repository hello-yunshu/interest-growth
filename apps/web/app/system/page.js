...
export default function SystemPage(){
  ...
  const health=useMemo(()=>[
    ...
    {label:'当前兴趣',title:area?.name||'加载中',detail:area?.domain_name||''},
  ...
  return <div className="stack">
    ...
    {tab==='runtime'&&<><section className="card"><TaskRows tasks={runtimeTasks}/></section>{isLocalDesktop(desktop)?<><FineTunePanel title="DeepSeek · 可选模型通道" fields={[{label:'模型地址（Base URL）',node:<input value={providerSettings.deepseekBaseUrl||''} onChange={e=>setProviderSettings({...providerSettings,deepseekBaseUrl:e.target.value})}/>},{label:'模型名称（Model）',node:<input value={providerSettings.deepseekModel||''} onChange={e=>setProviderSettings({...providerSettings,deepseekModel:e.target.value})}/>},{label:'API Key',node:<input type="password" autoComplete="new-password" placeholder={secretStatus?.configured?'已安全保存；留空保持不变':'输入 API Key'} value={deepseekSecret} onChange={e=>setDeepseekSecret(e.target.value)}/>}]} actions={<div className="row"><StatusChip tone={secretStatus?.configured?'success':'neutral'}>{secretStatus?.configured?'密钥已保存':'未配置密钥'}</StatusChip>{secretStatus?.configured&&<button className="ghost danger" disabled={saving} onClick={removeSecret}>删除 Key</button>}</div>}/><div className="row"><button className="secondary" disabled={saving} onClick={async()=>setDesktop(await restartDesktopCore())}>重启 Core</button><button disabled={saving} onClick={saveProvider}>{saving?'保存中…':'保存并重启 Core'}</button></div></>:desktop?.desktop?isAndroid?<p className="notice">Android 客户端通过自托管服务器接入。模型通道由服务器侧配置，本机不保存本地凭据。</p>:<p className="notice warning">当前处于自托管服务器模式。模型通道由服务器侧配置，本机不再显示本地凭据管理。切换到本机模式可管理本机模型通道。</p>:<p className="notice">安全凭据管理仅在桌面运行时显示。</p>}</>}
    {tab==='plugins'&&<RecordsTable columns={[{key:'manifest',label:'插件',render:r=><strong>{r.manifest.name}</strong>},{key:'enabled',label:'状态',render:r=><StatusChip tone={r.enabled?'success':'neutral'}>{statusLabel(r.enabled?'enabled':'disabled')}</StatusChip>},{key:'action',label:'操作',render:r=><button className="tableLink" onClick={()=>togglePlugin(r)}>{r.enabled?'停用':'启用'}</button>}]} rows={plugins}/>}
    {tab==='features'&&<RecordsTable columns={[{key:'name',label:'功能'},{key:'enabled',label:'状态',render:r=><StatusChip tone={r.enabled?'success':'neutral'}>{statusLabel(r.enabled?'on':'off')}</StatusChip>},{key:'action',label:'操作',render:r=><button className="tableLink" onClick={()=>toggleFeature(r)}>切换</button>}]} rows={features}/>}
    {tab==='area'&&<RecordsTable columns={[{key:'plugin_id',label:'能力'},{key:'enabled',label:'当前兴趣状态',render:r=><StatusChip tone={r.enabled?'success':'neutral'}>{statusLabel(r.enabled?'enabled':'disabled')}</StatusChip>},{key:'action',label:'操作',render:r=><button className="tableLink" onClick={()=>toggleAreaCapability(r)}>切换</button>}]} rows={areaCapabilities}/>}
  </div>
}
