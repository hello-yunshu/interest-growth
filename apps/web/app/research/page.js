...
export default function ResearchPage() {
  ...
  function approvePlan() {
    ...
    setPlan(current=>({...current,subquestions,approved:true}));setPlanApproved(true);setMessage('研究计划已确认；下次执行会使用这份快照。');
  }
  async function run(e) {
    e?.preventDefault?.(); if (busy) return; if(!researchAvailable){setMessage('深入研究当前未启用。仍可整理本地证据，但不能执行研究。');return;} if(!planApproved||!plan){setMessage('先生成并确认研究计划，再开始研究。');return;} setBusy(true); setMessage('');
  ...
  async function addSource(e) {
    ...
    try {
      ...
      setSourceForm({title:'',canonical_url:'',source_type:'paper'}); setMessage('来源已加入。打开原文核验后，再标记为“已核验”。'); await loadWorkspace();
    } catch (err) { setMessage(toUserMessage(err)); } finally { setBusy(false); }
  }
  async function verifySource(id) { if (busy) return; setBusy(true); try { await api(`/sources/${id}/verify`,{method:'POST'}); setMessage('来源已完成人工核验。'); await loadWorkspace(); } catch (err) { setMessage(toUserMessage(err)); } finally { setBusy(false); } }
  async function invalidateSource() {
    ...
    try {
      ...
      setMessage(`来源核验已撤销。${data.affected_claim_ids?.length||0} 条主张已进入再核验。`);
  ...
  async function addClaim(e) {
    ...
    try {
      ...
      setClaimForm({statement:'',limitations:'',confidence:0.5,publishability:'internal_only'}); setSupportIds([]); setCounterIds([]); setMessage('主张 v1 已创建。'); await loadWorkspace();
    } catch (err) { setMessage(toUserMessage(err)); } finally { setBusy(false); }
  }
  async function skepticPass(id){ if (busy) return; setBusy(true); try {const data=await api(`/claims/${id}/skeptic-pass`,{method:'POST'});setSkepticReviews(prev=>({...prev,[id]:data.review}));setMessage(`反向检查：${statusLabel(data.review.status)}。它不会替代人工核验。`);}catch(err){setMessage(toUserMessage(err));}finally{setBusy(false);} }
  ...
  async function reviseClaim(e){e.preventDefault();if(busy)return;setBusy(true);try{await api(`/claims/${revision.claim_id}/revisions`,{method:'POST',body:JSON.stringify({statement:revision.statement,limitations:revision.limitations,reason_for_revision:revision.reason_for_revision,publishability:revision.publishability,supporting_evidence:supportIds,contradicting_evidence:counterIds})});setMessage('新版本已保存，旧版本仍保留。上一版本的核验状态已失效，请重新进行反向检查并完成人工核验。');setRevision({...revision,claim_id:'',statement:'',limitations:'',reason_for_revision:''});setSupportIds([]);setCounterIds([]);await loadWorkspace();}catch(err){setMessage(toUserMessage(err));}finally{setBusy(false);}}
  ...
  return <div className="stack">
    ...
    <div className="card">
      <div className="fieldGroup">
        ...
        {!topics.length && <p className="help">先在「好奇心」中把一个问题转为长期主题，再创建主张和证据。</p>}
      </div>
      <div className="subcard sectionTop">
        ...
        <p className="muted">选择资料库后，深入研究会优先检索本地可重建索引；当前领域能力由本地服务提供。</p>
        <div className="fieldRow" style={{marginTop:8}}>
          <div><div className="fieldLabel">资料库</div><div className="checkGrid">{knowledgeBases.map(k=><label className="check" key={k.id}><input type="checkbox" checked={selectedKbIds.includes(k.id)} onChange={()=>toggleKb(k.id)}/><span>{k.name}</span></label>)}</div></div>
      ...
      <div className="stack sectionTop"><div className="row"><select aria-label="研究深度" className="compactSelect" value={depth} onChange={e=>{setDepth(e.target.value);setPlan(null);setPlanApproved(false)}}><option value="light">轻量</option><option value="normal">标准</option><option value="deep">深入</option></select><span className="muted">{researchAvailable ? '先生成候选理解，再进入人工证据链。' : '研究执行当前关闭；本地来源、证据和主张账本仍可维护。'}</span></div><PromptBar value={question} onChange={value=>{setQuestion(value);setPlanApproved(false)}} onSubmit={generatePlan} disabled={busy || !researchAvailable} placeholder="输入需要研究的问题…" model={energyLabel(depth)} context={selectedKbIds.map(id=>knowledgeBases.find(k=>k.id===id)?.name).filter(Boolean)} sendLabel="生成研究计划"/></div>
      ...
      {planApproved&&<div className="row sectionTop"><button onClick={run} disabled={busy||!question.trim()}>{busy?'正在执行…':'执行已确认的研究计划'}</button><span className="muted">研究结果仍是候选理解；来源、证据和主张继续由你人工维护。</span></div>}
      {message && <p className="notice">{message}</p>}
    </div>

    {busy && <PixelLoader label="正在生成候选理解" detail={energyLabel(depth)}/>}
