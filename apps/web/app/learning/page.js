...
export default function LearningPage() {
  ...
  async function mastery(id, state) { if (busy) return; setBusy(true); try { await api(`/concepts/${id}/mastery`, { method: 'PUT', body: JSON.stringify({ state, evidence_note: '' }) }); await loadTopic(); } catch (error) { setMsg(toUserMessage(error)); } finally { setBusy(false); } }
  ...
  const assistAvailable = capability.available('FEATURE_KNOWLEDGE_RAG', 'capability.concept-graph');
