...
export default function LearningPage() {
  ...
  async function createConcept(event) {
    ...
    try { await api('/concepts', { method: 'POST', body: JSON.stringify({ topic_id: topic || null, ...listFields(form), name: form.name, definition: form.definition }) }); setForm(emptyConcept); await loadTopic(); setMsg('概念已保存；掌握状态由当前领域的掌握标准决定。'); }
  ...
  async function saveEdit(event) {
    ...
    try { await api(`/concepts/${editing}`, { method: 'PUT', body: JSON.stringify({ ...listFields(editForm), name: editForm.name, definition: editForm.definition, related_claims: editRelated.claims, related_sources: editRelated.sources }) }); setEditing(''); await loadTopic(); setMsg('概念卡已更新，关联关系也会刷新。'); }
    catch (error) { setMsg(toUserMessage(error)); } finally { setBusy(false); }
  }
  ...
  async function assist(id, type) {
    ...
    try { const result = await api(`/concepts/${id}/${type}`, { method: 'POST', body: JSON.stringify({ knowledge_base_ids: selectedKb, focus: '' }) }); if (type === 'visualize') { const preview = await api(`/visual-artifacts/${result.artifact.id}/preview`); setVisual(preview); setMsg('可视化已保存，可在当前页面重新查看；使用前仍需人工审核。'); } else setMsg((result.result?.text || '学习辅助已生成。').slice(0, 700)); }
    catch (error) { setMsg(toUserMessage(error)); } finally { setBusy(false); }
  }
