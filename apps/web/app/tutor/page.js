...
import { capabilityLabel, statusLabel, toUserMessage } from '../../lib/presentation.js';
...
export default function TutorPage(){
  ...
  async function createSession(e){e.preventDefault();try{const x=await api('/tutor/sessions',{method:'POST',body:JSON.stringify({...form,topic_id:form.topic_id||null,concept_id:form.concept_id||null})});await load();setSessionId(x.id);setMsg('会话已保存在当前兴趣，并由本地服务执行。');}catch(e){setMsg(toUserMessage(e))}}
  async function connect(){if(!sessionId)return;try{const health=await api('/native-execution/health');setConnected(true);setMsg(health.llm_available?'本地导师服务已就绪。':'本地执行服务已连接；请在设置中配置 DeepSeek API Key 以启用 AI 导师。')}catch(e){setConnected(false);setMsg(toUserMessage(e))}}
  async function sendTurn(){if(!connected){setMsg('请先连接本地导师服务。');return;}if(!content.trim())return;const input=content.trim();setEvents([]);setPending(null);setRunning(true);setContent('');try{const x=await api(`/tutor/sessions/${sessionId}/native-turns`,{method:'POST',headers:{'X-PG-Native-Session':sessionId},body:JSON.stringify({capability,content:input})});setActiveTurnId(x.turn?.id||'');setEvents(x.events||[]);const wait=(x.events||[]).find(e=>e.category==='wait_for_input');setPending(wait||null);if((x.events||[]).some(e=>e.category==='error'))setMsg('本地导师服务执行失败，请检查模型配置。');await loadTurns()}catch(e){setMsg(toUserMessage(e))}finally{setRunning(false)}}
  ...
  async function cancel(){if(!activeTurnId)return;try{await api(`/tutor/sessions/${sessionId}/native-turns/${activeTurnId}/cancel`,{method:'POST',headers:{'X-PG-Native-Session':sessionId}});setPending(null);await loadTurns()}catch(e){setMsg(toUserMessage(e))}finally{setRunning(false)}}
  ...
  return <div className="stack">
    ...
    <div className="grid two">
      <section className="card"><div className="cardHeader"><div><div className="eyebrow">会话设置 · {domain?.area?.domain_name||'兴趣领域配置'}</div><h2>建立学习会话</h2></div><StatusChip>{sessions.length} 个已保存会话</StatusChip></div><form className="stack" onSubmit={createSession}><input aria-label="学习会话标题" value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/><select aria-label="学习主题" value={form.topic_id} onChange={e=>setForm({...form,topic_id:e.target.value})}><option value="">不限定 主题</option>{topics.map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select><select aria-label="学习概念" value={form.concept_id} onChange={e=>setForm({...form,concept_id:e.target.value})}><option value="">不限定 概念</option>{concepts.filter(x=>!form.topic_id||x.concept.topic_id===form.topic_id).map(x=><option key={x.concept.id} value={x.concept.id}>{x.concept.name}</option>)}</select><select aria-label="导师风格" value={form.persona_name} onChange={e=>setForm({...form,persona_name:e.target.value})}><option value="">不使用 导师风格</option>{personas.map(x=><option key={x.id} value={x.name}>{x.name}</option>)}</select>
        ...
        <div className="row" style={{justifyContent:'flex-end',marginTop:4}}><button className="buiButton--secondary">保存本地学习会话</button></div></form></section>
      <section className="card"><div className="cardHeader"><div><div className="eyebrow">继续会话</div><h2>已有学习会话</h2></div></div><select aria-label="选择学习会话" value={sessionId} onChange={e=>{setSessionId(e.target.value);setConnected(false);setActiveTurnId('')}}><option value="">选择学习会话</option>{sessions.map(s=><option key={s.id} value={s.id}>{s.title||s.id} · {statusLabel(s.status)}</option>)}</select><div className="row sectionTop"><button onClick={connect} disabled={!sessionId}>连接本地执行服务</button><button className="ghost" onClick={cancel} disabled={!activeTurnId}>取消当前对话</button></div><ChatPanel messages={messages} empty="这个学习会话还没有历史对话。"/></section>
    </div>
