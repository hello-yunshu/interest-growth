...
import { revisionModeLabel, statusLabel, toUserMessage } from '../../lib/presentation.js';
...
export default function WritingPage(){
 ...
 async function save(){if(busy||!id||!bundle)return;setBusy(true);try{const x=await api(`/writing/documents/${id}`,{method:'PUT',body:JSON.stringify({content_markdown:bundle.document.content_markdown,title:bundle.document.title})});setBundle({...bundle,document:x});setMsg('正文已保存。')}catch(e){setMsg(toUserMessage(e))}finally{setBusy(false)}}
 async function propose(mode=edit.mode){if(busy||!id||!edit.selected_text.trim())return;setBusy(true);try{const r=await api(`/writing/documents/${id}/revisions`,{method:'POST',body:JSON.stringify({...edit,mode,tools:[]})});await detail();setEdit(v=>({...v,mode}));setMsg(`修改提案已提出：${r.engine}。不会自动覆盖正文。`)}catch(e){setMsg(toUserMessage(e))}finally{setBusy(false)}}
 ...
 return <div className="stack">
   ...
   {bundle?.revisions?.length>0&&<section className="stack"><div className="sectionTitle"><div><div className="eyebrow">修改记录</div><h2>每次 AI 修改都有决策记录</h2></div></div>{bundle.revisions.map(r=><DiffTable key={r.id} before={r.selected_text} after={r.replacement_text} title={`${revisionModeLabel(r.mode)} · ${statusLabel(r.status)}`} meta={r.engine} actions={r.status==='proposed'?<><button onClick={()=>decide(r,true)} disabled={busy}>{busy?'处理中…':'接受修改'}</button><button className="ghost" onClick={()=>decide(r,false)} disabled={busy}>{busy?'处理中…':'拒绝修改'}</button></>:<StatusChip tone={r.status==='accepted'?'success':'neutral'}>{statusLabel(r.status)}</StatusChip>}/>)}</section>}
 </div>
}
