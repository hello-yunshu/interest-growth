...
export default function GrowthPage() {
  ...
  return <div className="stack growthPage">
    ...
    <div className="grid two">
      <section className="card reflectionIntro"><div className="eyebrow">长期记忆</div><h2>{hasLongTerm ? '已经出现一条长期变化线索' : '让总结等到材料足够时再发生'}</h2><p className="muted">{hasLongTerm ? '这不是给你贴标签，而是一份仍会被新经验修正的回顾。' : '系统不会为了制造成长感而强行总结。真实记录会慢慢形成可以回看的线索。'}</p><button className="secondary" onClick={openMemoryGraph} disabled={busy}>{busy?'正在整理…':'查看记录如何连接'}</button></section>
