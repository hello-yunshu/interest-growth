...
const META = {
  ...
  'research-outputs': { title: '研究产出', description: '已形成版本记录的研究结论。', icon: 'search' },
...
function QuestionsWidget({ data }) { const rows = data.dashboard?.recent_questions || []; return rows.length ? <div className="linkRows">{rows.slice(0, 4).map(row => <Link href="/curiosity" key={row.id}><Icon name="question"/><span><strong>{row.question}</strong><small>{energyLabel(row.energy_mode)} · 已回来 {row.returned_count || 0} 次</small></span><Icon name="arrowRight"/></Link>)}</div> : <EmptyWidget icon="question" text="还没有待继续的问题。" href="/curiosity" action="记下一个问题"/>; }

function OldNoteWidget({ data }) { const notes = data.notes?.notes || []; const note = notes.length > 2 ? notes[notes.length - 1] : notes[0]; return note ? <div className="oldNote"><Icon name="quote" size={28}/><blockquote>{note.body_markdown || note.title}</blockquote><strong>{note.title}</strong><Link href="/learning">重新打开 <Icon name="arrowRight"/></Link></div> : <EmptyWidget icon="pen" text="写下第一条笔记，之后可以在这里重新找到它。" href="/learning" action="写一条笔记"/>; }

function ResearchCheckWidget({ data }) { const claims = (data.claims?.claims || []).map(normalizeClaimRecord); const pending = claims.filter(row => row.verification_state !== 'human_verified'); return <MetricList value={pending.length} label="条结论待确认" rows={pending.slice(0, 3).map(row => row.statement || '尚未写下结论')} href="/research" empty="目前没有待确认的结论。"/>; }
...
function ReviewQueueWidget({ data }) { const rows = data.practice?.practice?.filter(bundle => !bundle.attempts?.length) || []; return <MetricList value={rows.length} label="项内容待复习" rows={rows.slice(0, 3).map(row => row.item?.prompt)} href="/learning" empty="目前没有待复习内容。"/>; }
...
function sourceState(value) { return String(value || '').includes('verified') ? '已人工确认' : '待确认'; }
