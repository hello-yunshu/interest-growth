...
const labels = {
  ...
  mastery: {
    unfamiliar: '还不熟悉', familiar: '接触过', explain: '能解释', example: '能举例',
    distinguish: '能区分', transfer: '能迁移', evidence_boundary: '知道证据边界', stable_expression: '表达稳定',
    understand: '开始理解', practice: '能练习', apply: '能应用', reflect: '能反思', self_directed: '能自主推进',
  },
  verification: {
  ...
  publishability: {
    ...
    stable: '相对稳定', controversial: '有争议', not_publishable: '不建议公开',
  },
  task: {
  ...
  connection: {
    Initializing: '正在初始化', Connected: '已连接', Reconnecting: '正在重新连接', Offline: '已离线',
    LoginExpired: '登录已过期', IdentityChanged: '服务器实例已变化', UpdateRequired: '需要更新',
    UnsupportedServer: '服务器版本不受支持', LocalCoreError: '本地服务出错',
  },
...
const REVIEW_ISSUE_LABELS = {
  no_supporting_evidence: '暂无支持证据，不能进入人工核验。',
  missing_supporting_evidence: '引用的部分支持证据已不存在。',
  ...
  no_counter_or_boundary_evidence: '暂无相反或边界证据，请确认是否存在条件、例外或争议。',
  missing_counter_evidence: '部分相反或边界证据已不存在，需要重新核对。',
  missing_limitations: '主张未写明限制或适用边界。',
  confidence_exceeds_evidence_base: '当前证据数量不足以支持这么高的置信度。',
  absolute_language: '表述可能过于绝对，请确认研究是否支持这样的表述强度。',
...
const RISK_REVIEW_LABELS = {
  language_risk: '这段表达需要按当前领域规则进一步检查。',
  ...
  claim_has_no_counter_evidence: '引用主张暂无相反或边界证据。',
  no_human_verified_evidence: '当前领域要求事实表达形成经人工核验的证据链。',
  no_counter_evidence: '当前内容暂无相反或边界证据记录。',
};
...
export function toUserMessage(error, context = {}) {
  ...
  if (code === 'FEATURE_DISABLED') return '这项功能已关闭。可在设置中重新启用。';
  ...
  if (code === 'RATE_LIMITED' || status === 429) return '请求过于频繁。请稍后再试。';
  if (code === 'FORBIDDEN' || status === 403) return '当前账号无权执行此操作。';
  if (status === 404) return '找不到这项内容。它可能已被移动或归档。';
  if (status === 409) return '当前内容已发生变化。请刷新后再试。';
  if (status === 422 || code === 'VALIDATION_ERROR') return '输入内容需要补充或调整。请检查后再试。';
  ...
  if (code === 'INVALID_EXTERNAL_URL') return '链接无法使用。请输入以 http 或 https 开头的公开链接。';
  if (code === 'UNSUPPORTED_EXTERNAL_URL') return '暂不支持这种链接类型。请输入 http 或 https 链接。';
  if (code === 'DISABLED') return '这项能力目前未启用。可在设置中重新打开。';
  return context.fallback || '操作未完成。请稍后重试。';
}
